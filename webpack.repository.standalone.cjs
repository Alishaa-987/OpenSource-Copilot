...
module.exports = {
  mode: 'production',
  target: 'node',
  entry: path.join(root, 'apps/repository-service/src/main.ts'),
  output: {
    path: path.join(root, 'dist/apps/repository-service'),
    filename: 'main.js',
    clean: false,
  },
  ...
  module: {
    rules: [{
      ...
      use: {
        loader: 'ts-loader',
        options: {
          configFile: path.join(root, 'apps/repository-service/tsconfig.app.json'),
          transpileOnly: true,
        },
      },
    }],
