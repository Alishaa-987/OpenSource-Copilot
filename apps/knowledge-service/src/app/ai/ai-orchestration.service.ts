...
// Groq's chat-completions endpoint rejects oversized request bodies with a
// plain HTTP 413 (Payload Too Large) before the model ever sees the prompt.
// The issue dossier (up to 60,000 chars), each retrieved chunk's content,
// and up to 10 history messages (up to 8,000 chars each) were all being
// sent verbatim, so a real-world issue + a few chunks + a longer chat could
// easily exceed that limit. These caps keep the outgoing payload well
// within Groq's limit while still giving the model plenty of grounded
...
export class AiOrchestrationService {
  ...
  async ask(repositoryId: string, question: string, cookieHeader: string, issueDossier?: string, history: readonly AskHistoryMessage[] = []): Promise<AskResult> {
    ...
    const recentHistory: ChatMessage[] = history.slice(-MAX_HISTORY_MESSAGES).map((message) => ({ role: message.role, content: this.truncate(message.content, MAX_HISTORY_MESSAGE_CHARS) }));
  ...
  private dossierPrompt(issueDossier: string | undefined, chunks: readonly RetrievedChunk[]): string {
    const dossier = issueDossier?.trim() ? this.truncate(issueDossier.trim(), MAX_DOSSIER_CHARS) : 'No structured issue dossier was supplied.';
  ...
  private contextPrompt(chunks: readonly RetrievedChunk[]): string {
    return "UNTRUSTED_REPOSITORY_DATA_START\n" + chunks.map((chunk) => "SOURCE path=" + chunk.path + " url=" + chunk.url + " relevance=" + chunk.relevance.toFixed(3) + "\n" + this.truncate(chunk.content, MAX_CHUNK_CONTENT_CHARS)).join("\n\n---\n\n") + "\nUNTRUSTED_REPOSITORY_DATA_END";
  }

  // Cuts text to a hard character cap and marks that it was cut, rather than
  // silently dropping context. Keeping this in one place means every prompt
  // section (dossier, chunk content, history) stays bounded the same way.
