...
export const knowledgeEnvSchema = baseEnvSchema.merge(kafkaEnvSchema).merge(redisEnvSchema).extend({
  ...
  QDRANT_COLLECTION: z.string().regex(/^[a-zA-Z0-9_-]{1,64}$/).default('opensource_repository_chunks_local_minilm'),
  EMBEDDING_PROVIDER: z.enum(['http', 'local']).default('local'),
  EMBEDDING_BASE_URL: z.string().url().optional(),
  EMBEDDING_API_KEY: z.string().min(1).optional(),
  EMBEDDING_MODEL: z.string().min(1).max(200).default('text-embedding-3-small'),
  EMBEDDING_DIMENSIONS: z.coerce.number().int().min(1).max(8192).default(384),
